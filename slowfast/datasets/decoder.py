...
def temporal_sampling(frames, start_idx, end_idx, num_samples):
	...
	index = torch.clamp(index, 0, frames.shape[0] - 1).long() #might take a frame for multiple times
	return torch.index_select(frames, 0, index)
...
def pyav_decode(
	...
	if duration is None or multi_clip_num > 1:
		# If failed to fetch the decoding information, decode the entire video.
		# If trying to take several clips from a single video, decode the entire video
		decode_all_video = True
...
def decode(
	...
	# Perform temporal sampling from the decoded video.
		multi_clip.append(temporal_sampling(frames, start_idx, end_idx, num_frames))
