...
def flatten(tensor, cfg):
	...
	if len(tensor) > 1:
		tensor[0] = tensor[0].view(-1, cfg.DATA.INPUT_CHANNEL_NUM[0], CF // cfg.SLOWFAST.ALPHA, H, H)
		tensor[1] = tensor[1].view(-1, cfg.DATA.INPUT_CHANNEL_NUM[1], CF, H, H)
	else:
		tensor[0] = tensor[0].view(-1, cfg.DATA.INPUT_CHANNEL_NUM[0], CF, H, H)
	return tensor
